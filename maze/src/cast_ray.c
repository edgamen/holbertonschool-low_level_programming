...
void cast_ray(float ray_angle, Player_POV *player, char (*map)[MAP_WIDTH], Line *line)
{
    ...
    if (raw_distance == -1) {
        if (DEBUG_DEFAULT) printf("find_distance reports no wall\n");
        line->start = 0;
        line->end = 0;
        return;
    }
    ...
    if (DEBUG_DEFAULT) printf("distance: %f, skewed_distance: %f\n", raw_distance, skewed_distance);
    col_length = (CUBE_LENGTH / skewed_distance) * PROJECTION_PLANE_DISTANCE;
    if (DEBUG_DEFAULT) printf("wall slice (column) length: %f\n", col_length);
    ...
    if (DEBUG_DEFAULT) printf("========== FIND_DIRECTION END, direction: %c ==========\n", line->direction);
