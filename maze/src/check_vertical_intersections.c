    ...
    Player_POV *player, char (*map)[MAP_WIDTH])
{
    ...
    else if (ray_facing_right)
    {
        ...
        if (DEBUG_DEFAULT) printf("warning! ray_angle %f is over 360 degrees, did not handle the case\n", ray_angle);
        return 0;
    }
    ...
    if (DEBUG_DEFAULT) printf("====== END CHECK_VERTICAL_INTERSECTIONS; found_wall = %i, x %f, y %f ===== \n",
